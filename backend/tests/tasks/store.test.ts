...
beforeEach(() => {
  ...
  taskStore = new TaskStore(new FileSystemStorageProvider(tmpDir));
});
...
describe('readAll', () => {
  ...
  it('returns all tasks sorted by sort_order ascending', async () => {
    ...
    const tasks = await taskStore.readAll({ sort: 'sort_order:asc' });
  ...
  it('filters by status', async () => {
    ...
    const tasks = await taskStore.readAll({ status: 'todo' });
  ...
  it('filters by tags (OR logic)', async () => {
    ...
    const tasks = await taskStore.readAll({ tags: 'work,personal' });
    expect(tasks).toHaveLength(2);
  ...
  it('filters by priority', async () => {
    ...
    const tasks = await taskStore.readAll({ priority: 'high' });
    expect(tasks).toHaveLength(1);
    expect(tasks[0].id).toBe('aaa11111');
  });
  ...
  it('searches title', async () => {
    ...
    const tasks = await taskStore.readAll({ search: 'groceries' });
    expect(tasks).toHaveLength(1);
    expect(tasks[0].id).toBe('aaa11111');
  });
  ...
  it('searches tags', async () => {
    ...
    const tasks = await taskStore.readAll({ search: 'shopping' });
    expect(tasks).toHaveLength(1);
    expect(tasks[0].id).toBe('aaa11111');
  });
});
...
describe('readById', () => {
  it('finds task by ID prefix in filename', async () => {
    ...
    const task = await taskStore.readById('abc12345');
  ...
  it('returns null when task not found', async () => {
    const task = await taskStore.readById('notexist');
...
describe('create', () => {
  it('creates a file with generated ID and correct frontmatter', async () => {
    const task = await taskStore.create({ title: 'New Task', due_date: '2099-12-31', description: 'Do it.' });
  ...
  it('assigns sort_order = max + 1 in column', async () => {
    ...
    const task = await taskStore.create({ title: 'New Task', status: 'todo', due_date: '2099-12-31', description: 'x' });
    expect(task.sort_order).toBe(6);
  });

  it('accepts explicit status', async () => {
    const task = await taskStore.create({ title: 'My Task', status: 'todo', due_date: '2026-12-31', description: 'x' });
    expect(task.status).toBe('todo');
  });
});
...
describe('update', () => {
  it('updates specified fields without touching others', async () => {
    ...
    const updated = await taskStore.update('abc12345', { title: 'Updated' });
  ...
  it('updates updated_at timestamp', async () => {
    ...
    const updated = await taskStore.update('abc12345', { title: 'New' });
    expect(updated.updated_at >= before).toBe(true);
  });

  it('throws when task does not exist', async () => {
    await expect(taskStore.update('notexist', { title: 'x' })).rejects.toThrow('not found');
  });
});
...
describe('updateStatus', () => {
  it('changes status and resets sort_order to bottom of target column', async () => {
    ...
    const updated = await taskStore.updateStatus('aaa11111', 'done');
  ...
  it('throws if moving to todo without due_date', async () => {
    ...
    await expect(taskStore.updateStatus('abc12345', 'todo')).rejects.toThrow('due_date');
  });

  it('accepts due_date when moving to todo', async () => {
    writeTask(makeTask({ id: 'abc12345', title: 'Task', status: 'done', due_date: undefined }));
    const updated = await taskStore.updateStatus('abc12345', 'todo', '2026-12-31');
...
describe('updateOrder', () => {
  it('renumbers sort_order for all tasks in column', async () => {
    ...
    await taskStore.updateOrder('ccc33333', 0);
    const tasks = await taskStore.readAll({ status: 'todo', sort: 'sort_order:asc' });
...
describe('deleteTask', () => {
  it('removes the task file', async () => {
    ...
    await taskStore.deleteTask('abc12345');
    const task = await taskStore.readById('abc12345');
    expect(task).toBeNull();
  });

  it('throws when task does not exist', async () => {
    await expect(taskStore.deleteTask('notexist')).rejects.toThrow('not found');
  });
});
...
describe('file watcher cache synchronization', () => {
  it('updates cache dynamically when file is created or deleted externally', async () => {
    ...
    const found = await taskStore.readById('ext12345');
    ...
    const deletedFound = await taskStore.readById('ext12345');
