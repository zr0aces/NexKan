...
backend/
  src/
    routes/
      ...
      telegram.ts         ← webhook receiver
      notifications.ts    ← POST /api/notifications/check
    services/
      ...
      telegram-bot.ts     ← grammy bot: send messages + handle commands
      notifier.ts         ← find due/overdue tasks, dispatch Telegram alerts
    middleware/
      webhook-auth.ts     ← validate X-Telegram-Bot-Api-Secret-Token header
    types/
      task.ts             ← shared Task interface
    app.ts
    server.ts
  data/
    tasks/                ← markdown files (Docker volume mount)
...
2. For each task with `due_date`:
   - If `due_date` within now+24h ±30min → send "Due in 24 hours" (if not already sent for key `{taskId}:due-24h`)
   - If `due_date` within now+1h ±10min → send "Due in 1 hour" (if not already sent for key `{taskId}:due-1h`)
   - If `due_date` < now → send "Overdue by X" (if not already sent for key `{taskId}:overdue:{YYYY-MM-DD}`)
3. Update `notifications-sent.json`
...
backend     ← Express API + Telegram webhook handler (port 3000, internal)
```
...
```nginx
# Telegram webhook bypasses auth
...
Backend validates `X-Cron-Secret` header on `/api/notifications/check`.
...
- Telegram bot integration (grammy, webhook mode)
- `docker-compose.yml` + `Dockerfile` (frontend build + backend)
